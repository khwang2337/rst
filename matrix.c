...
struct matrix * make_rotX(double theta) {
  struct matrix * xR = new_matrix(4, 4);
  theta = (M_PI/180) * theta;
  
  ident(xR);
  
  xR->m[1][1] = cos(theta);
  xR->m[1][2] = -1 * sin(theta);
  xR->m[2][1] = sin(theta);
  xR->m[2][2] = cos(theta);
...
struct matrix * make_rotY(double theta) {
  struct matrix * yR = new_matrix(4, 4);
  theta = (M_PI/180) * theta;
...
struct matrix * make_rotZ(double theta) {
  struct matrix * zR = new_matrix(4, 4);
  theta = (M_PI/180) * theta;
  
  ident(zR);
  
  zR->m[0][0] = cos(theta);
  zR->m[0][1] = -1 * sin(theta);
  zR->m[1][0] = sin(theta);
  zR->m[1][1] = cos(theta);
